...
$(function () {
    ...
    $.each(json.products, function (i) {
        ...
        if (i < 10) {
            var newProduct = $("body").find("#products > div.col").clone();

            newProduct.find("#product-image").append('<img src="' + this.imageS + '" class="image">')

            newProduct.find("#title").append(this.name);
            if (this.params.land) {
                newProduct.find("#category").append(this.params.land);
                if (this.params.region) {
                    newProduct.find("#category").append(" | " + this.params.region);
                    if (this.params.art) {
                        newProduct.find("#category").append(" | " + this.params.art);
                    }
                }
            }
            newProduct.find("#prices").append('<span class="price col">' + this.priceText + '</span>');
            if (this.oldPrice) {
                newProduct.find("#prices").append('<span id="old-price" class="old-price  col"><del>' + this.oldPriceText + '</del></span>');
                var discountPercentage = Math.round(((this.oldPrice - this.price) / this.oldPrice) * 100);
                if (!isNaN(discountPercentage)) {
                    newProduct.find("#discount").append('<span class="discount-box">' + discountPercentage + '% </span>');
                }
            }
            newProduct.find("#base-price").append(this.params.basePrice);



            if (this.params.likeCount) {
                newProduct.find("#like").append('<span class="like-box">' + this.params.likeCount + '<i class="far fa-heart"></i> </span>');
            }

            if (this.params.isNew == "true") {
                newProduct.find("#new").append('<span class="new-box"> NEU </span>');
            }

            $(newProduct).appendTo(".first");
            return true;
